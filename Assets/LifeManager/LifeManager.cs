...
namespace Rito
{
    ...
    /// <para/> * 
    /// <para/> * 1. 생성
    /// <para/> * * 1) 인스턴스화 또는 오브젝트 풀에서 로드 : Instantiate, Create(obj, pos, rot, parent)
    ...
    public class LifeManager : RitoSingleton<LifeManager>
    {
        ...
        /// <para/> 오브젝트 풀 내에 존재하지 않을 경우 새롭게 생성하여 리턴한다.
        /// </summary>
        public GameObject Instantiate(in GameObject target, in Vector3 pos, in Quaternion rot = default,
            in Transform parent = null)
        {
            return Create(target, pos, rot, parent);
        }
